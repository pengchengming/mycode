...
public class JsonToObjectUtil {
	...
	public static Object toObject(ArrayList ja, Class clz/*, String[] dataFormats*/)
			throws JSONException {
		BeanWrapperImpl bw = new BeanWrapperImpl(clz);
//		JSONArray namesArray = ja.names();

		for (int j = 0; j < ja.size(); j++) {
			JSONObject jo  = (JSONObject) ja.get(j);
//			Object value = jo.(name);
//			try {
//				setProperty(bw, name, value/*, dataFormats*/);
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
	 ...
	 * */
	@SuppressWarnings("rawtypes")
	public static Object toObject(String jsonString, Class clz/*,
			String[] dataFormats*/) throws JSONException {
	...
	@SuppressWarnings("rawtypes")
	public static void setProperty(BeanWrapper beanWrapper, String propName,
			Object value/*, String[] dataFormats*/) {
		// 得到.所在的位置
		...
		if (indexOfDot < 0) {
			if (beanWrapper.getPropertyType(propName).equals(
				...
				try {
					...
					/*	value = new SimpleDateFormat(dataFormats[i]).parse(value.toString());*/	
					/*	break;*/
					/*}*/
		...
		for (int j = 0; j < namesArray.length(); j++) {
			...
			try {
				if(name.equals(colletionName)){//对应的名称是数组
					...
					List values=JsonToObjectUtil.toList(value.toString(), colletionClz);
					JsonToObjectUtil.setProperty(bw, name, values);
				}else{
					JsonToObjectUtil.setProperty(bw, name, value);
				}
