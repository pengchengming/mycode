		...
		if(judge==1){//审批通过
			...
			$.ajax({
				...
				data:{
					...
					condition: " status in(4202,5204) and  request_id="+id
				},
				complete : function(xhr, textStatus){
					var data = JSON.parse(xhr.responseText);
					var results = data.results;
...
function print_atta(param,me){
	  if(param == 1){
	       ...
	       window.open(rootPath + "/hmcheck/printcompany?requreid="+id+"&step=5");
	  }else if(param == 2){
		  $("#printcompany").attr("class","");
		  $("#printatta").attr("class","active");
		  window.open(rootPath + "/hmcheck/printatta?requreid="+id+"&step=5");
	  }	 
}
	...
	$.ajax({
		...
		complete : function(xhr, textStatus){
			...
			if(data.code + "" == "1"){
				...
				adddataobj["register"] = {"status":51041,"returnRemark":returnRemark,"isReturn":1,"approval_Date":requestinfo.nowDate,"approvalStep":5,"request_Id":id,"companyId":requestinfo.companyId};
					...
					$.each(requestUsers,function(i,requestUser){
						...
						if(userApprovalIds&&userApprovalIds.length>0){
							$.each(userApprovalIds,function(i,userApprovalId){
								if(userApprovalId.approvalStep + "" == "4" && (userApprovalId.status + ""== "4202" || userApprovalId.status + ""== "4203")){
									var obj = new Object();
									obj["formId"]= 67;
									obj["tableDataId"] = userApprovalId.id;
									obj["register"] = {"status":4204};
					...
					$.ajax({
						...
						data : { 
							...
							var data = JSON.parse(xhr.responseText);
								if(data.code+""=="1"){
									alert("退回成功");
									window.location.href=rootPath+"/talentoffice/rcbApprovalList?menuId=67";
							 	}else {
									alert(data.errorMsg);
							}	
						}});
                                ...
                                <div class="panel-body no-padding-top">
                                	...
                                	<input type="hidden" id="materailstate" value="0" />                       		
                               		...
                               		<table id="approvalstep">
                               			<tr><td class="text-right">资格终审结果:</td>
                               			...
                               			<td colspan="3">
                   							...
                   							<input type="button" onclick="openlayer()" class="btn btn-info margin-right-10" value="退回申请">&nbsp;&nbsp;							
                               			</td>
                               			</tr>
                               		</table>
                               		</div>
